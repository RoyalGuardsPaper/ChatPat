...
def detect_canonical_context(query: str) -> bool:
    """
    Detect whether the query likely refers to a well-known person,
    body of work, or established topic without hard-coding entities.
    """
    return bool(re.search(r"by\s+[A-Z][a-z]+|works of|writings of|papers by|memos by", query, re.IGNORECASE))

def rank_by_consensus(results):
    """
    Rank repeated noun phrases across snippets as a proxy for importance.
    """
    phrase_counts = {}
    for r in results:
        words = re.findall(r"[A-Za-z][A-Za-z\-]{3,}", r["text"].lower())
        for w in words:
            phrase_counts[w] = phrase_counts.get(w, 0) + 1

    # Remove extremely common words
    stop = {
        "that","this","with","from","they","their","about",
        "which","will","have","been","were","more","most"
    }

    ranked = [
        (k, v) for k, v in phrase_counts.items()
        if k not in stop and v > 1
    ]
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        if canonical_context and importance_requested and grounding_results:
            consensus = rank_by_consensus(grounding_results)
            if consensus:
                consensus_text = (
                    "CONSENSUS SIGNALS (terms repeated across sources):\n"
                    + "\n".join(f"- {k} (mentioned {v} times)" for k, v in consensus)
                )
                messages.append(
                    {
                        "role": "system",
            ...
            with st.expander("🔗 Sources"):
                for r in grounding_results:
                    ...
                    label = " (authority)" if any(
                        d in src for d in ["wikipedia.org", "oaktreecapital.com", "ft.com", "bloomberg.com"]
                    ) else ""
